...
TEST(test_algo, get_in_out_vals) {
  ...
  auto algo_ptr = std::make_unique<neb::rt::graph_algo>();

  auto addr_and_in_out_vals = algo_ptr->get_in_out_vals(graph);
...
TEST(test_algo, merge_edges_with_same_from_and_same_to) {
  ...
  auto graph = tg.internal_graph();
  auto algo_ptr = std::make_unique<neb::rt::graph_algo>();
  algo_ptr->merge_edges_with_same_from_and_same_to(graph);
...
TEST(test_algo, merge_topk_edges_with_same_from_and_same_to_less_than_k) {
  ...
  auto graph = tg.internal_graph();
  auto algo_ptr = std::make_unique<neb::rt::graph_algo>();
  algo_ptr->merge_topk_edges_with_same_from_and_same_to(graph);
...
TEST(test_algo, merge_graphs) {
  ...
  auto algo_ptr = std::make_unique<neb::rt::graph_algo>();
  auto ptr = algo_ptr->merge_graphs(v);
...
TEST(test_algo, merge_two_graphs) {
  ...
  neb::rt::transaction_graph_ptr_t ptr2 =
      std::make_unique<neb::rt::transaction_graph>(tg2);
  auto algo_ptr = std::make_unique<neb::rt::graph_algo>();
  auto friend_ptr = std::make_unique<neb::rt::friend_graph_algo>();
  friend_ptr->merge_two_graphs(algo_ptr.get(), ptr1.get(), ptr2.get());
...
TEST(test_algo, remove_cycles_based_on_time_sequence_case1) {
  ...
  auto graph = tg1.internal_graph();
  auto algo_ptr = std::make_unique<neb::rt::graph_algo>();
  algo_ptr->non_recursive_remove_cycles_based_on_time_sequence(graph);
