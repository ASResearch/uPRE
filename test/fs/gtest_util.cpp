...
TEST(test_util, concate_name_version) {
  std::string name = "name";
  neb::version_t version = 1;
  auto ret =
      neb::number_to_byte<neb::bytes>(version) + neb::string_to_byte(name);
  EXPECT_EQ(ret, neb::concate_name_version(name, version));

  ret = neb::number_to_byte<neb::bytes>(version) + name;
  EXPECT_EQ(ret, neb::concate_name_version(name, version));
}
