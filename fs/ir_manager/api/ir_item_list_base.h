...
public:
  ...
  virtual bytes get_ir_brief_key_with_height(block_height_t height) {
    ...
    if (items.empty()) {
      ...
      throw std::runtime_error(
          "ir_item_list_base::get_ir_brief_key_with_height: items empty");
    }
    ...
    if (pos == items.begin()) {
      if (pos->template get<p_start_block>() == height) {
        return concate_name_version(m_ir_name, pos->template get<p_version>());
      } else {
  ...
  virtual nbre::NBREIR find_ir_at_height(block_height_t height) {
    ...
    if (items.empty()) {
      LOG(ERROR) << "items empty";
      throw std::runtime_error(
          "ir_item_list_base::find_ir_at_height: items empty");
    }
    ...
    if (pos == items.end()) {
      pos--;
      return get_ir(pos->template get<p_version>());
    }
