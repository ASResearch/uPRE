...
class nebulas_rank_algo {
  friend class friend_nebulas_rank_algo;
...
protected:
  virtual transaction_graph_ptr_t build_graph_from_transactions(
      const std::vector<neb::fs::transaction_info_t> &trans);

  virtual block_height_t get_max_height_this_block_interval(
      const std::vector<neb::fs::transaction_info_t> &txs);

  virtual floatxx_t f_account_weight(floatxx_t in_val, floatxx_t out_val);

  virtual floatxx_t f_account_rank(int64_t a, int64_t b, int64_t c, int64_t d,
                                   floatxx_t theta, floatxx_t mu,
                                   floatxx_t lambda, floatxx_t S, floatxx_t R);
};

class friend_nebulas_rank_algo {
public:
  virtual transaction_graph_ptr_t build_graph_from_transactions(
  ...
  virtual floatxx_t f_account_weight(nebulas_rank_algo *algo_ptr,
                                     floatxx_t in_val, floatxx_t out_val);

  virtual floatxx_t f_account_rank(nebulas_rank_algo *algo_ptr, int64_t a,
