...
transaction_graph_ptr_t friend_nebulas_rank_algo::build_graph_from_transactions(
    nebulas_rank_algo *algo_ptr,
    const std::vector<neb::fs::transaction_info_t> &trans) {
  return algo_ptr->build_graph_from_transactions(trans);
}

block_height_t friend_nebulas_rank_algo::get_max_height_this_block_interval(
    nebulas_rank_algo *algo_ptr,
    const std::vector<neb::fs::transaction_info_t> &txs) {
  return algo_ptr->get_max_height_this_block_interval(txs);
}

floatxx_t friend_nebulas_rank_algo::f_account_weight(
    nebulas_rank_algo *algo_ptr, floatxx_t in_val, floatxx_t out_val) {
  return algo_ptr->f_account_weight(in_val, out_val);
}

floatxx_t friend_nebulas_rank_algo::f_account_rank(
    nebulas_rank_algo *algo_ptr, int64_t a, int64_t b, int64_t c, int64_t d,
    floatxx_t theta, floatxx_t mu, floatxx_t lambda, floatxx_t S, floatxx_t R) {
  return algo_ptr->f_account_rank(a, b, c, d, theta, mu, lambda, S, R);
}
} // namespace nr
} // namespace rt
} // namespace neb
